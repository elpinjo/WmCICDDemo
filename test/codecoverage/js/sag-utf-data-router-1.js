...
(function () {
    ...
    app.controller('treeTable-datarouter', [
        ...
        function (
            ...
            $scope.fullPackDataloaded = false;

            var getPage = function () {
                var url;
                $rootScope.pagecontext = $routeParams.param;
                //$scope.gridOptions = $scope.gridOptions_fp;
                ...
                switch ($routeParams.param) {
                    case "fullpackageview":
                        ...
                        break;
                    case "executionmodel":
                        if ($scope.execPathDataloaded)
                           // getExecutionModelViewPage();
                        break;
                    case "servicelevel":
                        if ($scope.packServicesDataloaded )
            ...
            var packagesummary = function () {
                ...
                overallsummary.overall_pkg_childcount = pkgs_instruction.childCount;
                overallsummary.overall_pkg_visitedchildcount = pkgs_instruction.visitedChildCount;
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $scope.dataloaded = result != undefined;
                    $scope.fullPackDataloaded = result != undefined;
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $scope.packServicesDataloaded = result != undefined;
                },
                error: function (hr,status,error) {
                    //$scope.dataloaded = false;
                    $scope.packServicesDataloaded = false;
                }
            });
            ...
            if ($scope.dataloaded) {
                $scope.packServicesDataloaded = elementNSCascadedResult.includeServiceLevelReport;
                getPage();
            }
    ...
    app.controller('treeTable-datarouter-em', [
        '$rootScope', '$scope', '$routeParams', 'uiGridConstants', 
        'pb$dataservice', 'td$dataservice', 'em$dataservice', 'imageproviderservice',  'CommaFormatted', 
        '$interval', '$timeout',
        function (
            ...
            $rootScope.pagecontext = "executionmodel";
            $scope.data_location = data_location;
            ...
            $scope.gridOptions = {
                ...
                rowHeight: 25,
                columnDefs: [
                    { name: 'Elements', width: '100%', field: 'displayText', 
                        cellTemplate: 
                        "<div class=\"ui-grid-cell-contents\" style=\"cursor:default;\" title=\"TOOLTIP\">" + 
                        "<div" + 
                        "style=\"float:left; width:100%\" class=\"ui-grid-tree-base-row-header-buttons\"" + 
                ...
                onRegisterApi: function (gridApi) {
                    ...
                    $scope.gridApi.core.on.rowsRendered($scope, function () {
                        ...
                        $scope.$watch('data', function () {
                                ...
                                var row = $scope.gridApi.grid.rows[0];
                                if (!$scope.initialized && row != undefined) {
                                    //selecting the row
                                    $scope.gridApi.treeBase.toggleRowTreeState(row);
                                    $scope.gridApi.treeBase.expandRow(row);
                                    $scope.initialized = true;
            ...
            var getExecutionModelViewPage = function () {
                ...
                {
                    $scope.overallsummary.title_cs = "Service Execution Model of each test";
                    $scope.overallsummary.coveragepercent_cs = overallsummary.overall_tst_coveragepercent_cs;
                    $scope.overallsummary.overall_coveragepercent = overallsummary.overall_tst_coveragepercent;
                    $scope.overallsummary.overall_childcount_cs = overallsummary.overall_tst_childcount_cs;
                    $scope.overallsummary.overall_visitedchildcount_cs = overallsummary.overall_tst_visitedchildcount_cs;
                ...
                for (var index = startIdx; index < endIdx; index++) {
                    ...
                    $.ajax({
                        ...
                        error: function (result) {
                            var displayText = elements[0] + " [" + result.statusText + " - " + result.status + "]";
                            var errordata = "{";
                            errordata += "\"instruction\": {";
                            errordata += "\"displayText\": \"" + displayText  + "\",";
                            errordata += "\"Type\": \"TestCase\",";
                            errordata += "\"statusText\": \"" + result.statusText + "\",";
                            errordata += "\"status\": " + result.status;
                            errordata += "}}";
                            elementInstructionCoverage.push(JSON.parse(errordata));
            ...
            $scope.overallsummary = overallsummary;
            $scope.execPathDataloaded = testDescriptionsResult.includeExecutionPathReport;
            if ($scope.execPathDataloaded) {
                getPage();
            }
    ...
    app.controller('treeTable-datarouter-mock', [
        ...
        function (
            ...
            $timeout, $interval) {
                ...
                //$scope.dataloaded = false;
                $scope.mockDetailsDataloaded = false;
                var overallsummary = {};
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        //$scope.dataloaded = true;
                        $scope.mockDetailsDataloaded = true;
                    },
                    error: function (hr,status,error) {
                        //$scope.dataloaded = false;
                        $scope.mockDetailsDataloaded = false;
                ...
                //if ($scope.dataloaded) {
                if ($scope.mockDetailsDataloaded) {
                    $scope.overallsummary = overallsummary;
                    getPage();
                }
        ...
        if (testDescriptionsResult == undefined) {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    //$scope.dataloaded = true;
                    $scope.execPathDataloaded = true;
                },
                error: function (hr,status,error) {
                    //$scope.dataloaded = false;
                    $scope.execPathDataloaded = false;
                }
            });
        }
